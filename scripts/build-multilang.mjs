...
/**
 * Multi-language build script for ReFi Provence
 * 
 * This script builds the Quartz site for multiple locales.
 * Configured for French (default) and English.
 * 
 * Structure:
 * - fr-FR (French) -> public/ and public/fr/ (default)
...
// Configure locales for ReFi Provence
// Format: { code: "locale-code", prefix: "url-prefix", isDefault: true/false }
const LOCALES = [
  { code: "fr-FR", prefix: "fr", isDefault: true },
...
function buildForLocale(locale, prefix, isDefault) {
  ...
  if (isDefault && prefix !== "fr") {
    ...
    if (existsSync("public")) {
      ...
      const dirsToCopy = ["about", "events", "partners", "join-community", "contact", "how-to-support-us"]
...
function mergeStaticAssets() {
  ...
  // Also add default locale prefix if not "fr"
  const defaultLocale = LOCALES.find(l => l.isDefault)
  if (defaultLocale && defaultLocale.prefix !== "fr") {
...
function createRootRedirect() {
  ...
  const defaultLocale = LOCALES.find(l => l.isDefault)
  const defaultPrefix = defaultLocale ? defaultLocale.prefix : "fr"
...
<body>
  ...
  writeFileSync(join(rootDir, "public", "index.html"), redirectHTML)
...
async function main() {
  ...
  LOCALES.forEach(locale => {
    if (locale.isDefault && locale.prefix !== "fr") {
